...
class UserController extends Controller
{
    ...
    public function login()
    {
        if(Auth::attempt(['email' => request('email'), 'password' => request('password')])) {
            $user = Auth::user();
            $tokenResult = $user->createToken('Gratiser Android App');
        ...
        try{
            ...
            Mail::send('email.register_success', $data, function($message) use ($to_name, $to_email) {
                $message->to($to_email, $to_name)
                        ->subject('Welcome to Gratiser');
                $message->from('noreply@gratiser.com','Gratiser');
            });
        }catch(Exception $e){
            $return = [
            ...
            if($requestData["password"] != $user->password){
                try{
                    ...
                    Mail::send('email.password_changed', array(), function($message) use ($to_name, $to_email) {
                        $message->to($to_email, $to_name)
                                ->subject('Gratiser Password Changed');
                        $message->from('noreply@gratiser.com','Gratiser');
                    });
                }catch(Exception $e){
                    $return = [
    ...
    private function notifyChangedPassword($user){      
        ...
        $notificationBuilder->setTitle('Gratiser Password Changed')
                            ->setBody('Your Gratiser password has been changed')
                            ->setSound('default')
                            ->setBadge('https://gratiser.oriadesoft.id/images/gratiser_logo_square.png');
